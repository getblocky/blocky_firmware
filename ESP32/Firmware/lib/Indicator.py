...
class Indicator :
	...
	async def animate(self , name):
		await core.asyn.NamedTask('indicator-handler').cancel ()
		if name == 'ota-success':
			@core.asyn.cancellable
			async def handler (self):
				for x in range(255):
					self.rgb.fill((x,x,x));n.write()
			...
			pass
		core.mainthread.create_task ( core.asyn.NamedTask('indicator-handler',self.handler) )
	
	async def show (self , function):
		try :
			await core.asyn.NamedTask.cancel('indicator-handler')
		except Exception as err :
			print('[Indicator] -> {}'.format(err))
		while core.asyn.NamedTask.is_running('indicator-handler') :
			await core.asyncio.sleep_ms(10)
		core.mainthread.create_task( core.asyn.NamedTask('indicator-handler',function)()    )
